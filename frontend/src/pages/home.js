...
export default function Home() {
  ...
  function startVoiceRecognition() {
    ...
    recognition.lang = 'en-IN'; // Change to 'hi-IN' for Hindi or other supported language
  ...
  // Send transcript to backend endpoint for order parsing and handling
  async function sendTranscriptToBackend(transcript) {
    try {
      const  response = await api.post('/voice-order', { transcript });
      console.log('Voice order response:', response.data);
      // Optionally handle confirmation or feedback here
    } catch (err) {
      setError('Error sending voice order.');
    }
  }
        ...
        <div className="max-w-screen">
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
              ...
              >
                🎤 {isListening ? t('home.listening') : t('home.voiceOrder')}
              </button>
            </div>
            {/* Voice transcript display */}
            {voiceTranscript && (
              <div style={{ marginTop: '12px', background: '#e0ffe0', borderRadius: '8px', padding: '8px' }}>
                <strong>{t('home.voiceTranscript')}:</strong> {voiceTranscript}
              </div>
            )}
