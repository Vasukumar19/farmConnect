...
// WhatsApp integration removed — notification calls replaced with no-ops

// Create Order (Customer only)
// Create Order with WhatsApp notification
const createOrder = async (req, res) => {
  try {
    ...
    // Populate for WhatsApp notifications and response
    const populatedOrder = await orderModel.findById(newOrder._id)
      .populate('product', 'name unit price')
      .populate('farmer', 'name phone')
      .populate('customer', 'name phone');

    // WhatsApp notifications removed. Previously sent notifications are no-ops now.
    // If you want to re-enable notifications, restore the WhatsApp service and call it here.
...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    // WhatsApp status updates removed — no external notification will be sent here.
...
// Cancel Order with WhatsApp notification
const cancelOrder = async (req, res) => {
  try {
    ...
    // order.customer may be populated (object) or an ObjectId. Normalize to string id.
    const customerId = order.customer && order.customer._id ? order.customer._id.toString() : order.customer.toString();
  ...
  // Cancellation notification via WhatsApp removed.
...
const sendPaymentReminder = async (req, res) => {
  try {
    ...
    // WhatsApp payment reminders are disabled in this repository.
    res.json({ success: false, message: 'WhatsApp integration removed; no payment reminder sent' });
  } catch (error) {
    console.error('Error sending payment reminder:', error);
    res.json({ success: false, message: 'Failed to send payment reminder' });
  }
};
