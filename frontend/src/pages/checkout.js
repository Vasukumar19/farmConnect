...
export default function Checkout() {
  ...
  const createOrdersForCartItems = async () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (paymentMethod === 'razorpay') {
      if (!RAZORPAY_KEY_ID) {
        alert('Razorpay is not configured. Please add REACT_APP_RAZORPAY_KEY_ID to your .env file.');
        return;
      }
      setPlacing(true);
      try {
        const { success, order: razorpayOrder } = await createRazorpayOrder(amountInPaise);
        if (!success || !razorpayOrder?.id) {
        ...
        const options = {
          ...
          currency: 'INR',
          name: 'FreshConnect',
          description: 'Farm Connect Order',
          ...
          handler: async (response) => {
            try {
              ...
              if (verifyResult.success) {
                await createOrdersForCartItems();
                await clearCart();
            ...
            <div className="card" style={{ marginTop: '24px' }}>
              ...
              <div className="input-group">
                ...
                >
                  ...
                  <option value="razorpay">{t('payment.razorpay')}</option>
